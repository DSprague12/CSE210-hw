public class Journal{
    ...
    public void fetchJournal(string fileName){
        ...
        foreach (string line in lines){
            journalStage.Add(deobfucate(line));
        }
    }
    ...
    public void saveJournal(string fileName){
        using(StreamWriter finalFile = new StreamWriter(fileName)){
            foreach (string[] item in journalStage){
                finalFile.WriteLine($"{obfucate(item[0])},{obfucate(item[1])},{obfucate(item[2])}");
            }
        }
    }

    //Converts from bit64 to plaintext ;)
    string[] deobfucate(string data){
    ...
    public string obfucate(string plainText){
